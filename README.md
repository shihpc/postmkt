...
- `build_postmkt.py`：抓 FinMind dataset＋TWSE 公開端點的最新交易日全市場資料，
  ...
  **宇宙＝當日 `TaiwanStockPrice` ∩ `TaiwanStockInfo`（`nm`）－ 商品類黑名單**，兩者建置時
  ...
  （2026-09-09 CI 實測 45,675 列，權證佔絕大多數），照單全收會讓全檔從 ~1.65MB 變成
  ...
  那些代號會在前端呈現成「查無此代號」而非「資料源不涵蓋」。黑名單只擋三類
  （`RE_MARKET_CODE` 要求數字開頭、`RE_MARKET_EXCLUDE` 擋權證與 ETN）：**權證**
  （6 碼 `03`–`09` 開頭／`7` 開頭；`nm` 自己就收了 36 檔，實測 `710553` 當日有價，
  所以 `nm` 單獨當閘門擋不住）、**ETN**（`nm` 內 48 檔，FinMind 實際代號是 `020041`／`02001L`
  這種不帶 U 的寫法）、**`Index`／`大盤` 偽代號**（`TAIEX`、`Semiconductor`… 共 32 筆）。
  取捨方向固定：寧可多放幾檔（幾十列、不到 1KB），不可少放一檔。
  **已知代價（需前端配合）**：ETN 是可被持有的證券，被黑名單排除後前端會顯示「查無此代號」
  ——文案語意（已下市／停牌／代號有誤）與事實不符，前端需另備「資料源不涵蓋」的說法。
  收斂後約 2,700–2,800 檔（個股＋ETF＋DR＋REIT＋特別股全保留，含 `00637L`／`00981A`
  這類字母後綴），區塊約 55KB。
