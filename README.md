...
- **`f`／`t` 為 `null` ≠ 無異動——不得當成 0，也不得說成「無顯著異動」**。
  ...
  （寧缺勿混；`build_postmkt.py` grep `法人資料日與本區塊基準日`，**唯一命中**——同檔**裸名
  `法人資料日` 有 3 處命中**（2026-09-10 重數更正，原寫 2 處），三處**以「是什麼」指認、不記行號**：
  ①`build_market_daily()` 的 docstring（**就是上面那個錨點所在的那一行，它自己也算一次**，
  這正是前三次同型錯誤漏算的那一種）、②守門觸發時的 `print`、③`main()` 裡 `md_date` 上方的說明註解。
  **本節刻意不再寫出現值行號**（2026-09-13 決定）：這三個行號在**同一天內被迫重量四次**
  （:549／:573／:787 → `53c5ac0` 的 :558／:582／:796 → `3cec758` 的 :571／:595／:809 →
  `85be131` 的 :580／:604／:818 → 再一次更正 TOP_N 敘述後又漂），**每一次都不是程式邏輯改變，
  而是我們自己在同一個檔裡加註解造成的**——連「修行號」這件事本身都會再把行號推走。
  結論不是「下次記得重量」而是**不要記現值行號**：判讀一律用上面那個 grep 宣告式錨點。
  數法＝`grep -o 法人資料日 build_postmkt.py | wc -l` ＝ 3；本例 `grep -c`（**算行數**）恰巧也是 3，
  因為沒有任何一行出現兩次——**兩者巧合相等、不可互相取代**。計數範圍**僅限 `build_postmkt.py`**
  （全 repo 另有 README／CLAUDE.md／index.html／tests／CHANGELOG 等多處，不在此數內）。守門測試
...
- **第五軸：`market_daily.date` 不是「今天」——畫面主語不得用「今天／今日／當日」代稱**
  ...
  `build_postmkt.py` 的 `dates = [...]`／`latest = max(dates)`（**錨點 grep `dates = [d for d in (d_margin`；
  本節刻意不寫行號**——2026-09-13 一天之內它就被迫重量四次：:768-769 → :777-778 → :790-791 →
  :799-800 →⋯，**每一次都只是因為同一個檔裡有人加了註解**，連「修行號」的那次修改自己也推走了它。
  **行號會漂、宣告式不會**），**只取 margin／lend／
  short／dt／block／inst／hold 七支 FinMind dataset 的日期**，**不含**兩支 TWSE 零股日期
  `d_oddi`／`d_odda`（錨點 grep `d_oddi, r_oddi = `），
  所以寫成「所有資料源的最大日」是過寬的。
