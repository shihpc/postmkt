...
- **第五軸：`market_daily.date` 不是「今天」——畫面主語不得用「今天／今日／當日」代稱**
  ...
  `market_daily.date` ＝ `build_market_daily(lend_date, …)` 傳進去的 `lending.date`（價格／借券
  資料日），**與 `data/postmkt.json` 最上層的 `date`（頂列顯示的全檔基準日）語意不同、值也常常不同**
  ——線上 `44ef7e7` 實測頂列 `2026-09-09`、本區塊 `2026-09-08`，**系統性差一天**。同一畫面同時
