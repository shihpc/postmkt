...
- **`f`／`t` 為 `null` ≠ 無異動——不得當成 0，也不得說成「無顯著異動」**。
  ...
  （寧缺勿混；`build_postmkt.py` grep `法人資料日與本區塊基準日`（**唯一命中**，:580）——同檔**裸名 `法人資料日` 有 3 處命中**（2026-09-10 重數更正，原寫 2 處）：:580 docstring（**就是上面那個錨點所在的那一行，它自己也算一次**，這正是前三次同型錯誤漏算的那一種）、:604 守門觸發時的 `print`、:818 `main()` 裡 `md_date` 上方的說明註解（**這三個行號在 2026-09-13 一天內重量過三次**：本節寫成 :549／:573／:787 時就已經是舊值——`53c5ac0` 當下實為 :558／:582／:796——`3cec758` 在 `build_lending()` 加 13 行註解後為 :571／:595／:809——本批 `85be131` 修 TOP_N 與 packfile 兩處註解、再淨增 9 行後成為現值。**同一天漂三次**，判讀時一律以上面那個 grep 宣告式錨點為準）。數法＝`grep -o 法人資料日 build_postmkt.py | wc -l` ＝ 3；本例 `grep -c`（**算行數**）恰巧也是 3，因為沒有任何一行出現兩次——**兩者巧合相等、不可互相取代**。計數範圍**僅限 `build_postmkt.py`**（全 repo 另有 README／CLAUDE.md／index.html／tests／CHANGELOG 等多處，不在此數內）。守門測試
...
- **第五軸：`market_daily.date` 不是「今天」——畫面主語不得用「今天／今日／當日」代稱**
  ...
  `build_postmkt.py:799-800` 的 `dates = [...]`／`latest = max(dates)`（2026-09-13 內三度更新：
  先由 :768-769 →:777-778（`build_lending()` 排序上方加註解），再→:790-791（同節的註解依驗收
  意見重寫、又長了 13 行），本批 `85be131` 再→:799-800（同函式的 TOP_N 與 packfile 兩處註解
  更正、淨增 9 行）。**這正是行號錨點的通病：同一天就漂了三次**——
  **行號會漂、宣告式不會**，要 grep 的話用 `dates = [d for d in (d_margin`），**只取 margin／lend／
  ...
  `d_oddi`／`d_odda`（`build_postmkt.py:796-797`，錨點用 `d_oddi, r_oddi = `），
  所以寫成「所有資料源的最大日」是過寬的。
