...
- `build_postmkt.py`：抓 FinMind dataset＋TWSE 公開端點的最新交易日全市場資料，
  ...
  **ETN 已納入**（2026-09-09 使用者裁定）：ETN 是可以被持有的證券（`nm` 內 48 檔，代號一律
  ...
  收斂後約 **2,730–2,830 檔**（個股＋ETF＋ETN＋DR＋REIT＋特別股全保留，含 `00637L`／`00981A`
  這類字母後綴），區塊約 55KB，全檔約 **1.70–1.72 MB**（本檔 MB 一律 10⁶ 進位；錨點＝2026-09-08
  基準日的真實建置 1,707,249 bytes ＝ 1.707 MB ＝ 1.628 MiB，見 CHANGELOG 2026-09-09）。
