...
- `build_postmkt.py`：抓 FinMind dataset＋TWSE 公開端點的最新交易日全市場資料，
  ...
  **ETN 已納入**（2026-09-09 使用者裁定）：ETN 是可以被持有的證券（`nm` 內 48 檔，代號一律
  ...
  ——2026-09-09 那次點預測就因此低估了 1,753 B，見 CHANGELOG 2026-09-09。
...
以上是「代號在不在表裡」這一軸。**另外兩軸同樣會把「缺資料」呈現成別的東西，一併釘住**：
...
- **`f`／`t` 為 `null` ≠ 無異動——不得當成 0，也不得說成「無顯著異動」**。
  ...
  同一列的 `chg`（漲跌%）那天仍然有效、來源是 `price_rows` 本身，可照常判讀，不必整檔靜音。
...
- **`rows` 為空或明顯殘缺 ＝「無法取得異動資料」，不得逐檔說成「查無此代號」**。
  ...
  這是三種狀態裡最嚴重的一種說錯話。**前端必須先看整表健康度**：`rows` 為空、或
