    ...
    if rows:
        def stock_line(r):
            ...
            return (f"{r['c']}{nm} 集中10分{c10}×/30分{c30}× 漲跌{signed(r.get('chg'), 2)}%"
                    f" 投信連買{jsnum(r.get('it') or 0)}日/外資連買{jsnum(r.get('fi') or 0)}日 法人強度{ints}%")
