...
# **五天**的實際分布是**三失敗兩成功**（2026-09-13 補 09-11，原寫「四天三失敗一成功」已過期）：
#   09-07／09-08／09-09 摘要 batch 逾 180 分仍 in_progress → cancel、三筆全數 sync 回退
#     （整場 3h07／3h08／3h25）；
#   09-10 約 **104 分** ended → 三筆全數 batch（整場 1h47）；
#   09-11 **整場只有 7 分 32 秒**（run 34602845627，13:10:46Z→13:18:18Z）、三筆全數 batch
#     ——摘要 batch 因此**上界約 5 分鐘**。
# **變異比原本記的還大**：同一包三筆請求、同一個提交時段（UTC 13:1x），完成時間橫跨
# 約 5 分 ~ 104 分 ~ >180 分。這讓「不要為單一樣本調參」更站得住，也說明 09-10 那筆
# 104 分既不是常態也不是上界——**任何固定分鐘數都在賭一個分布極寬的量**。
# 所以固定分鐘數兩頭都不對：短了砍掉會成功的那天，長了（180 分）則在失敗的那天把產物推到
# 台北 00:1x~00:4x、晚於健檢，**失敗的那天**誤報一則「summary-pm(無檔)」
# （原寫「天天誤報」——與上面四天分布表自相矛盾，2026-09-10 覆驗指出，同型於本批要治的
# 「每天白等」那句全稱過頭）。
#
...
#
# 【★ 餘裕只有幾分鐘，而且取決於當晚幾點被 dispatch —— 這是本設計最脆弱的地方】
# （2026-09-10 覆驗算出）。取各班 job log 裡「呼叫 Anthropic…期限 N 分」**那一行的時戳**
# ——不是 `run_started_at`——換算新規則會給的期限，以及對照成功那包的 6240 秒：
#   09-07 21:11:25 → 108 分（餘 274 秒）／09-08 21:15:34 → 104 分（**餘 26 秒**）／
#   09-09 21:20:35 →  99 分（**短 275 秒，會被 cancel**）／09-10 21:11:19 → 108 分（餘 281 秒）。
# **若 09-10 照 09-09 的時刻起跑，新規則一樣會把它 cancel 掉**；而 09-08 那天的餘裕
# **只有 26 秒**——那是四天裡最能說明「這條線有多脆」的一筆。
# （**2026-09-11 更正**：本段初稿寫 109／105／100／109，四個值**全部多 1 分鐘**。錯因兩個且同向：
# ①拿 `run_started_at` 當基準，但 `batch_deadline()` 是在 job 起跑後 10~22 秒、過完齊全閘門與
# gather 才呼叫；②`dl = int(min(...))` 是**截斷**不是四捨五入。方向性結論沒變，但 09-08 由
# 「約 1 分鐘」變成「26 秒」，剛好落在會影響判斷的量級——所以這種數字要按實際呼叫點算。）
# 所以「牌鐘救得到 104 分那包」只在起跑夠早的日子成立，餘裕是**秒到數分鐘**等級、
# **不是結構性的保證**。起跑時刻由 v2 Worker 的晚場協調班決定
# （台北 21:00–23:55 每 5 分，事件驅動），不在本檔控制範圍。
# 之所以仍不再往後挪截止點：只有 N=1 筆成功樣本，為它調參就是對單一樣本過擬合——
# 那正是第一版「沒有代價」出錯的同一個機制。要動這個值，先累積幾天 `via` 分布。
#
# 【要往上／往下調之前】先看幾天 -pm.json 的 `via` 欄（`sync`＝那包 batch 又沒趕上牌鐘）。
# 【副作用（刻意）】場次期限與牌鐘都是 per-slot、摘要與彙總共用，所以彙總那包同受這個牌鐘綁。
# **牌鐘生效的那天，彙總幾乎一定失去半價**：摘要被砍時已過 23:00 → 彙總這次算出的期限為 0 →
# 強制走同步（`run_synthesis` 的 `if deadline_sec:` 分支）。這是換來的、不是漏掉的——
# 但代價要講明：09-09 那天彙總其實是 batch 成功的（1214 秒），新規則下它會改付原價。
# **「幾乎」不是修辭**（2026-09-11 覆驗指出，原寫「必然」是錯的）：cutoff 綁在
# `n.replace(hour=23, minute=0)`＝**`n` 當下的日曆日**，所以三筆同步回退若耗掉 >60 分而跨過午夜，
# `n` 換日、cutoff 跳到隔日 23:00，`remain` 反而變成約 23 小時 → 彙總又拿得到非零期限、照走 batch。
# 那個 >60 分的門檻正是下一段自己算出來的「三筆最壞約 60 分」，**兩段本來就互相指涉**，
# 寫「必然」等於和自己打架。實測四天是 4 分 17 秒~4 分 53 秒，離 60 分很遠，屬敘述問題非行為問題。
# **另一條沒寫的擠壓**：摘要**險勝**的日子彙總同樣吃虧——09-10 若照新規則，摘要 22:55 才完成，
# 彙總只剩約 5 分期限（那天實跑 202 秒、勉強過關）；而 09-09 的彙總實跑 **1214 秒**，
# 同樣的量在 5 分期限下會被 cancel。所以彙總被擠壓有兩條路，不只「摘要被砍」那一條。
# 【「留 50 分鐘足夠」的邊界】那句是拿**實測值**（摘要同步 4 分 17 秒）算的，不是上界。
# `call_claude` 是 `timeout=(15, 600)`＋重試一次，單筆同步的理論上界約 20 分，三筆最壞約 60 分，
# 從 23:00 起算會落到 00:00、超過健檢也超過 SYNC_RESERVE_SEC（15 分）。這是**既有**風險
# （不是本批引入），但別把「足夠」讀成保證。
