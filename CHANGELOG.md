...
**86／9 這個飽和值本身穩健**（三組互不相干的 seed 各取 ≥10 個樣本皆收斂到同一答案、9 名清單相同），
**但「要幾個樣本才收斂」不是性質、是那組 seed 的軌跡**：seeds 0–7（8 個）即到 86／9，
seeds 100–107（8 個）只到 **85／8**、要第 10 個才到。隨機 8-seed 子集約 26% 到不了 86。
（**這句原本寫成「5 個樣本量到 85、8 個以上飽和於 86」——那是把一組 seed 的軌跡寫成普適性質，
與本批一直在修的「數字比量到的更普適」同型，2026-09-13 覆驗退回後改寫。**）
