...
**體積與宇宙重新估算（取代上一節的 2,736 檔／~1,707,600 bytes）**
...
- **同一天同一份輸入下的預測**：2,724 ＋ 12（DR／REIT／`2887Z1`，`8dc1f0b` 放行的）
  ＋ **21（ETN）** ＝ **宇宙 2,757 檔**。
  兩組加項都是**本次逐檔實打 `TaiwanStockPrice&data_id=…&start_date=end_date=2026-09-08`**
  重新確認的（不是沿用上一節的數字）：34 檔中 12 檔當日有列
  （`01001T`／`01002T`／`01004T`／`01007T`／`01009T`／`2887Z1`／`910322`／`910861`／`911608`／
  `911622`／`911868`／`912000`，與上一節逐檔相符）；48 檔 ETN 中 **21 檔**當日有列
  （`020000`／`020001`／`020011`／`020012`／`02001L`／`020020`／`020025`／`020028`–`020041`）。
- **全檔預測**：新增 33 列，以 `json.dumps(separators=(",",":"))` 逐列實算＝
  858 B（`chg`/`f`/`t` 全 null）～990 B（`chg` 兩位小數＋四位數 `f`/`t`）→
  **1,708,100–1,708,240 bytes ≒ 1.708 MB**。
- **日常區間**：宇宙 **2,730–2,830 檔**、全檔 **1.70–1.72 MB**（ETN／DR／REIT 每天有無成交
  不同，其餘區塊本來就有逐日變動）。真實建置落在 2.7 MB 或 1.65 MB 附近都代表改動沒生效。
...
- **黑名單仍然必要（`nm` 單獨當閘門不夠）**：2026-09-09 拉 `TaiwanStockInfo` 全表實測，
  ...
  （「ETN」20＋「指數投資證券(ETN)」28）、以及 `Index`／`大盤` 的**產業別／指數偽代號 32 筆**
