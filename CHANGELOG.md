...
**但舊碼「差異看得見」的唯一形狀是「前 50 名代號集合 == `{某個 falsy 且非空字串的值}`」**
（本節脈絡是 `None`，但實測 `0`／`False` 同樣「舊碼不炸且 old≠new」——`or ""` 對**任何** falsy
值都有差；**空字串 `""` 是唯一的例外**，`.get(k,"")` 與 `or ""` 對它同值，old==new。
只要那個集合裡混進任一一般代號，舊碼就是 `TypeError`）。**注意不是「跑不到」**——覆驗構造出反例：一個 `None`
代號排在第 51 名時，舊碼**照樣走過** `tv.get(None)` 那條分歧路徑、不炸，只是被 `[:TOP_N]`
截掉所以 old／new 輸出仍逐位相同。「唯一」只成立在**可見**這一層。
