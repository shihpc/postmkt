...
以真實 `PYTHONHASHSEED` 走訪序產生多個舊序對跑 —— **排列會變的 86 個**（5 個樣本量到 85，
8 個以上飽和於 86）、**成員會變的 9 個**且清單穩定：`+`／`利`／`台新`／`宏`／`年`／`建`／
`復華`／`業`／`訊`。新行為下這些全部變成確定的。
...
- **「切片」不是順序消費的全部**：融借券整合排行主表 `tbl([...], allRows)` 沒有預設排序
  （`sort:{i:null}`），**畫面預設列序就是 JSON 列序、2,233 列全渲染**——那是最大的順序消費端，
  已記在本則上方「已知行為改變」那段。這裡用「切片」限定是為了分類，不是說只有切片會受影響。
