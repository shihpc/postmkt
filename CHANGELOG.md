...
**上述機制只對 `None` 成立**（因為 price_rows 缺 `stock_id` 產生的雜鍵就是 `None`）。
其餘 falsy 代號的實測結果如下——**這裡刻意只列實測、不下「任何」「唯一」這種全稱斷言**
（前四版就是每次補一個全稱、下一個邊緣案例再打破它，補到第四次還在漏；列表會過時，
全稱斷言則是直接錯）。場景＝前 50 名代號集合 == `{v}`、price_rows 有一列缺 `stock_id`：

| `stock_id` 的值 | 舊碼 | old vs new |
|---|---|---|
| `None` | 跑得過 | **不同**：`ratio`／`chg_pct`／`amp_pct` 由有值變 `null`（本段主旨的誤配機制） |
| `0`／`False`／`0.0` | 跑得過 | **不同，但只差 `c` 欄**（`0`→`""`）；`ratio` 等兩邊都是 `null`，**不是**上述機制 |
| `""` 與空字串的 `str` 子類 | 跑得過 | **相同**（`.get(k,"")` 與 `or ""` 對它同值） |
| `[]`／`{}`／`set()` | **`TypeError: unhashable type`** | 不適用——舊碼在 `tv.get(c)` 就炸了，連 `{v}` 這個集合都構造不出來 |
