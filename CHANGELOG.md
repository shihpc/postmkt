...
**正常資料回歸檢查——⚠ 這是回歸檢查，不是鑑別檢查，對本次改動的鑑別力是 0**
（實證：把修法故意改壞成 `c = r.get("stock_id") or "__WRONG__"`，同一量法輸出**與正確版
一字不差**）。原因：母體是 `market_daily` 的 2,753 個真實代號、**全部是非空字串**，而對
非空字串 `d.get(k,"")`／`d.get(k) or ""`／`d.get(k) or "任何值"` 三者恆等——要有鑑別力，
母體必須摻進顯式 `None`，**那正是守門測試在做的事（鑑別力由它提供，不是由這個量法提供）**。
它能證明的只有「正常資料路徑沒被改壞」，這仍有價值，但不可讀成「改動已被驗證」。

量法（臨時腳本不進 repo；**下列參數已實測足以重現**，照抄即得同一個 sha）：
母體＝**`git show e5d925e:data/postmkt.json`**（**必須釘住這個 blob**——`data/postmkt.json`
是受版控但**每個交易日被管線覆寫數次**的檔，2026-09-14 當天就寫了 2 次，照「當前檔案」跑
隔天就重現不出來；釘住後 2,753 列、資料日 `2026-09-14`）的 `market_daily.rows` 每列 index 0（`cols` 為
`["c","chg","f","t"]`），**依原順序**；`rng = random.Random(20260914)`；
**單一迴圈** `for i, c in enumerate(codes)`，每檔依序抽：
`vol = rng.choice([0,0,1,500,1000,250000])`（含 `vol<=0` 被濾的路徑）→
`amt = rng.choice([1e6,5e8,5e8,5e8])`（**Buy 與 Sell 共用這一次抽樣的同一個值**，
大量 `amt` 並列以壓排序穩定性）→ 加一列
`{"stock_id":c,"Volume":vol,"BuyAmount":amt,"SellAmount":amt}`；
**僅當 `i % 7`**（即 `i%7==0` 的檔刻意不給 price 列）再依序抽
`rng.choice([0,10000])`／`rng.choice([None,100.0])`／`rng.choice([None,1.5])` 組成
`{"stock_id":c,"Trading_Volume":…,"close":…,"spread":…,"max":101.0,"min":99.0}`。
以 `e060b49`（改動前）與本 commit 分別跑 `build_daytrading("", dt_rows, price_rows, {})`，
`json.dumps(out, ensure_ascii=False, sort_keys=True, separators=(",",":"))`
皆 **4,580 bytes、sha256 `aaef3f4345843c81…`**，逐位相同。

> **這一段被退回兩次，兩次都是同一型錯誤，留著當教訓**：
> ①初版寫「以真實 `by_amount` 50 列反推輸入再對跑，5,207 bytes／sha256 `04d72e25…`」
> ——那個 sha **重現不出來**（bytes 完全重現、sha 不同，最可能是反推在某個等長欄位還原
> 不精確），且該量法近乎恆真、50 列全是倖存者碰不到 `[:TOP_N]` 與 `vol<=0`。
> ②第二版（就是在修①的那次編輯）換成上面的合成量法，卻**在同一項上完全恆真**，
> 文字還寫「量法可重現」——實際上覆驗者照文字跑得到 4,574／4,583，**窮舉 32 種組合**
> 才補上沒寫出來的「Buy／Sell 共用一次抽樣」。**在宣告要修「近乎恆真」的段落裡，
> 端出一個完全恆真的替代品**，這是本批最該記住的一次。
> 現在的寫法：參數補全到可重現，並**明說它是回歸檢查、鑑別力為 0**。
...
**雜鍵是什麼值，取決於 price_rows 那列的 `stock_id`**——`tv`／`px_map`／`close_map` 都用
`r.get("stock_id")` 當鍵，**缺 key 時雜鍵是 `None`**，`stock_id` 顯式為 `0`／`False`／`0.0`
時雜鍵就是那個值，**同一個誤配機制照樣成立**（實測：兩種情形 old 的 `ratio`／`chg_pct`／
`amp_pct` 皆有值、new 皆 `null`。**這裡刻意不寫出那些值**——它們取決於 fixture 的
`Volume`／`Trading_Volume`，寫出來就又是一個不可重現的數字，而要被支持的宣稱是
「有值 vs `null`」，數字不承載任何重量）。所以下表是**在指定場景下的實測列舉，不是通則**。

**這裡刻意只列實測、不下「任何」「唯一」「只」這種全稱斷言**——前五版每一次都是下一個全稱、
被邊緣案例打破、補一層限定、再露新破口（第五次就是「上述機制**只**對 `None` 成立」，被
「price_rows 那列 `stock_id` 顯式為 `0`」推翻）。**列表會過時，全稱斷言則是直接錯。**

下表場景＝前 50 名代號集合 == `{v}`、**price_rows 有一列缺 `stock_id`**（故雜鍵為 `None`）；
範圍＝JSON 能產生的 falsy 值（`null`／`false`／`0`／`0.0`／`""`／`[]`／`{}`）＋兩個 Python 端才有的
對照（空字串 `str` 子類、`set()`）＋**一列不是「值」而是「`stock_id` 這個 key 不存在」**
（那是本修法的對照基準，以值為索引的列舉本來涵蓋不到它）：

| `stock_id` 的值 | 舊碼 | 新碼 | old vs new |
|---|---|---|---|
| `None` | 跑得過 | 跑得過 | **不同**：`ratio`／`chg_pct`／`amp_pct` 由有值變 `null`（本段主旨的誤配機制） |
| `0`／`False`／`0.0` | 跑得過 | 跑得過 | **不同，但只差 `c` 欄**（`0`→`""`）；`ratio` 等兩邊都是 `null`——本場景雜鍵是 `None`、對不到它們 |
| `""` 與空字串的 `str` 子類 | 跑得過 | 跑得過 | **相同**（`.get(k,"")` 與 `or ""` 對它同值） |
| **缺 `stock_id` 這個 key** | 跑得過 | 跑得過 | **相同**（`c` 皆 `""`）——這是本修法的對照基準：`.get` 的預設值對「缺 key」本來就有效 |
| `[]`／`{}`／`set()` | **`TypeError: unhashable type`**（炸在 `tv.get(c)`） | **不炸**，`c` 為 `""` | **這才是全表最大的差異**（old 崩潰 vs new 正常）。另註：這種值連 `{v}` 這個集合都寫不出來，所以上面那個「場景」在文字上也只是示意 |

**而且「差異看得見」還要那列進得了前 50**：一個 `None` 代號排在第 51 名時，舊碼**照樣走過**
`tv.get(None)` 那條分歧路徑、不炸，只是被 `[:TOP_N]` 截掉，old／new 輸出仍逐位相同。
而那個「有值」本身是拿缺代號的當沖列去對缺代號的價格列、
...
- **packfile 實測**：單版 base **約 306,103 B**；「同資料重跑、舊碼（單鍵）」增量 **≈4,375 B**；
...
**已知行為改變：異常日的並列順序會從「隨機」變成「代號升冪」**（不改程式，記為已知）。
像 `44ef7e7` 那種「TWSE 借券整批抓空、全表 `sys_bal+otc_bal` 並列 0」的日子，主鍵整欄並列，
次鍵 `c` 就成為唯一決定順序的鍵。兩個消費端受影響的程度**不同，不可混講**：
`build_summary.py` 的 `lr[:25]`（與 `index.html` `insightGatherContext()` 的 `lr.slice(0,25)`，
同口徑、同為送進 LLM 的摘要素材）取到的會從**隨機 25 檔**變成**代號最小的 25 檔**；
而**前端「融借券」tab 的整合排行渲染整張表、沒有任何截斷**——`index.html` 是
`tbl([...], allRows)` 且 `allRows = l.rows`，`tbl()` 只在列數超過 `VIRTUAL_THRESHOLD`
時做**虛擬捲動**（只決定當下畫幾列 DOM，不丟任何列），所以**整張 2,233 列的順序
都變成代號升冪**，影響面反而比原本記的更大。
...
**已知行為改變之二：借券 tab「個股查詢」的前 20 會變**（2026-09-13 第三版才數對，見下方更正史）

`index.html` `lendSearchAreaHtml()` 對 `allRows`（＝`d.lending.rows`）`filter` 保序後 `.slice(0,20)`，
是**使用者看得到的畫面**。**變化的方向是「由不確定變確定」，不是「從一個正確答案變成另一個」**：
全表 2,233 列中相鄰同鍵位置 568 個、836 列（37.4%）落在並列鍵上，而舊行為的前 20
**本來就取決於 `codes = set(...)` 的走訪序**，該走訪序由 `PYTHONHASHSEED` 決定、CI 未釘死
（`grep -rn PYTHONHASHSEED .github/` 零命中），**每個行程都不同**（實測 15 個 seed 得 15 個相異序）。

量法（不依賴任何一版舊輸出當基準）：候選查詢＝**代號與名稱各自**取 1~2 字子字串、
**轉小寫**後聯集（＝畫面 `q.toLowerCase()` 對 `c/n.toLowerCase()` 的比對口徑）共 **4,469** 個；
以真實 `PYTHONHASHSEED` 走訪序產生多個舊序對跑 —— **排列會變的 86 個、成員會變的 9 個**，
清單穩定：`+`／`利`／`台新`／`宏`／`年`／`建`／`復華`／`業`／`訊`。新行為下這些全部變成確定的。
**86／9 這個飽和值本身穩健**——覆驗者以 **120 個獨立隨機 seed** 量得 86／9、9 名清單相同（**這一項是
覆驗者的量測，我方未重跑該規模**；我方自己驗過的是下面三組小樣本）。
**但「要幾個樣本才收斂」完全不是性質，連 10 個樣本都不保證**：seeds 0–7（8 個）即到 86／9；
seeds 100–107（8 個）只到 85／8、要第 10 個才到；**seeds 7000–7009（10 個）只到 84／8**
——清單少一個「年」（此三組皆我方實測）。
**不要為「幾個樣本才夠」訂任何數字。**
（**兩次同型更正**：①初版寫「5 個樣本量到 85、8 個以上飽和於 86」；②改寫後又寫成「三組互不相干的
seed 各取 ≥10 個樣本皆收斂」——**後者是假的**，seeds 7000–7009 就是反例，而那句正是在修①的同一次
編輯裡寫下的。同批另刪掉一句「隨機 8-seed 子集約 26% 到不了 86」：那個數字**我方從未量過、是轉述**，
且照字面「隨機 8-seed」重做會得到約 32~34%（覆驗者量），26% 的真正母體是「上述兩組各 10 個 seed
內部窮舉 C(10,8)」——**母體沒寫出來的百分比等於不可重現**。這整串正是本批一直在修的
「數字比量到的更普適」，連修它的過程都又犯了兩次。）

> **更正史（這一條本身被數錯兩次，記著當教訓）**：v1（`53c5ac0`）寫「`lr[:25]` 與**前端排行榜的
> `TOP_N`**」——後者是虛構的，`grep -c TOP_N index.html` ＝ 0。v2（`fabd247`）改成「**恰有兩處**，
> 都是取前 25」——漏掉這一處。v3（`9d7c9b4`）數對了三處，但**候選查詢數 5,870 與它自己寫的定義
> 不符**（那個數字是把代號與名稱**串接後**再切、且未轉小寫得到的，會產生 `0台` 這種兩邊都不存在
> 的跨界字串，其中 1,387 個一列都比不中），現已改為 4,469。
> 另有一處主對話自己的誤判要記：v3 曾以「例子取決於拿哪一版舊序當基準」為由否定覆驗者舉的
> 「宏／復華／訊／年」——**那是錯的**，這四個全都在上面那 9 個穩定清單裡，任兩個相異輸入序之間
> 成員都會變。當時的隨機洗牌模型沒撞到，卻被當成對方的錯。**模型沒量到 ≠ 對方講錯。**

**先講清楚分類**：下面用「切片」限定，是為了分類、**不是說只有切片會受列序影響**。融借券整合排行
主表 `tbl([...], allRows)` 沒有預設排序（`sort:{i:null}`），**畫面預設列序就是 JSON 列序、
2,233 列一列都不截斷**（`rows.length > VIRTUAL_THRESHOLD`＝200 會走虛擬捲動，那只決定當下畫幾列
DOM、**不丟任何列**——所以不可說成「2,233 列全渲染」）。那才是最大的順序消費端，已記在上方
「已知行為改變」那段。
...
- **另外兩處消費列序的切片，輸出不變**：①`build_summary.py` 的 `lr[:25]`、
  ②`index.html` `insightGatherContext()` 的 `lr.slice(0,25)`。兩處都取前 25，而實測
  `data/postmkt.json` 排序鍵 `k[i] == k[i+1]` 在前 25 內零命中（前 50、前 100 內也是零），
  且**切點本身無跨界並列**（`k[24] != k[25]`、`k[19] != k[20]`——這才是「成員不變」真正依賴的條件，
  第一個相鄰並列出現在 index 133）。
  （**措辭更正**：②原寫「畫面標籤」是錯的——`insightGatherContext()` 產的是**送 LLM 的 prompt
  文字**，`dlabel(...)` 是 prompt 段落標題，全檔追 `g.text` 的去向沒有任何渲染到畫面的路徑。）
- **`c` 在 `lending.rows` 內實測唯一**（2,233 列、2,233 個相異 `c`、無 `None`），所以
  `find`／first-wins／last-wins 那幾類消費端（`stkFindC`／`buildBkNm`／`stockCodeSet`／
  `build_diag.py` 的 `pm_map`）與走訪序無關。
