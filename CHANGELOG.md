...
（＝所有資料源的最大日）脫鉤後常態相等，但只要有任何一支資料源比法人更新，兩者就會再度分開，
**相等不是保證**。脫鉤縮小的是發生頻率，不是消滅這個狀態——所以畫面主語仍然不得寫「今天」。
原本的「系統性差一天」實測（線上 `44ef7e7`）改寫成**脫鉤前**的成因證據保留，不刪除。

## 2026-09-10 pm 摘要 batch 期限 180 分 → 30 分（天天白等 3 小時、還天天誤報）
...
16:24:56  摘要完成：3/3 份成功          ← 同步回退只花 4 分 22 秒
...
**改法**：`BATCH_DEADLINE_SEC["pm"]` 180 分 → **30 分**（am 不動）。現況下沒有代價，
因為那包 batch 本來就沒成功過。**本常數是 per-slot、摘要與彙總共用**，所以彙總的上限也變 30 分
——比它現在分到的 25 分還寬，不是退步。預期產出從 00:4x 提前到約 21:5x，健檢自然通過。

**可觀測範圍的誠實邊界**：`-pm.json` 只留最近 3 日（管線刪 3 日前舊檔），`via` 欄只驗得到
09-07~09-09。09-02~09-04 只有落地時間（16:31／16:26／16:40 UTC），時間形狀一致，
**推斷同因、無直接證據**。

**要調回去之前，先看幾天 `-pm.json` 的 `via` 欄**（`sync`＝那包 batch 又沒趕上）。
