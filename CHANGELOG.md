...
「**每天**白等」也講得太滿——實際分布是**三失敗兩成功**
（**2026-09-13 補上 09-11，原寫「四天三失敗一成功」已過期**）：

| 日期 | 摘要 batch | 整場耗時 | 結果 |
|------|-----------|---------|------|
| 09-07／09-08／09-09 | 逾 180 分仍 `in_progress` → cancel | 3h07／3h08／3h25 | 三筆全 `sync` 回退（原價） |
| 09-10 | 約 **104 分** ended | 1h47 | 三筆全 `batch`（半價），產物台北 22:58 |
| **09-11** | **上界約 5 分** ended | **7m32s**（run `34602845627`） | 三筆全 `batch`，產物台北 **21:18** |

**09-11 那天健檢沒有響**（21:18 遠早於 23:50）——當時主對話預測「今晚會再報一次告警」，
那個預測**錯了**，因為它假設舊的 180 分碼一定會燒滿，而那天 batch 幾分鐘就完成。
同一包三筆、同一個提交時段（UTC 13:1x），完成時間橫跨約 **5 分 ~ 104 分 ~ >180 分**
——**分布極寬**，任何固定分鐘數都是在賭它，牌鐘的意義正在於不必賭。
...
起因是「持股異動」tab 的驗收句宣稱「375／390／1280 三寬度 `scrollWidth == innerWidth` 不溢出」，
獨立實測發現**那句把兩件事混成一件**——整頁的確沒有水平捲軸，但表格容器 `.tblbox` 自己在
375px 且法人欄為 6 位數張數時會溢出幾 px。

### 版面機制（實測，不是推論）

`.mtable` 是 `width:100%`（`index.html:79`）＋ `th,td{white-space:nowrap}`（`:80-81`），
所以**表格總寬先被鎖在容器寬、四欄互相重分配**；一路壓到 min-content 才停，之後由
`.tblbox`（`overflow:auto`，`padding:6px 12px 10px`，`:67-68`）橫捲吸收。
因此「會不會溢出」同時取決於**法人欄的位數**與**第一欄股名的長度**。

本批實測（Playwright Chromium headless，本機 `python -m http.server 8000`＋`page.route` 餵人工
fixture；fixture 改自本機 `data/postmkt.json`，`market_daily` 資料日 2026-09-08、2757 列；
持股 5 檔，`f`／`t` 三情境 × 股名長短兩組，各量 375／390／1280 三寬度，共 18 組）：

| 股名組 | `f`／`t` 情境 | 375px 表寬 | 375px `.tblbox` scroll/client | 375px 溢出 | 不溢出所需最小視窗寬 |
|---|---|---|---|---|---|
| 短（`0050 元大台灣50` 等 5 檔） | 全 `null`（線上現況） | 333.00 | 357/357 | 0 | 300 |
| 短 | 5 位數（±99,999） | 333.00 | 357/357 | 0 | 331 |
| 短 | 6 位數（±374,476） | 333.00 | 357/357 | 0 | 347 |
| 長（`00890B 凱基ESG BBB債15+` 等 5 檔，本機 `BK_NM` 最長股名 13 字） | 全 `null` | 333.00 | 357/357 | 0 | 333 |
| 長 | 5 位數 | 333.00 | 357/357 | 0 | 366 |
| 長 | 6 位數 | 339.48 | **363/357** | **+6px** | **381** |

- **18 組全部 `document.documentElement.scrollWidth == window.innerWidth`**（整頁無水平捲軸），
  **20 個儲存格逐格 `scrollWidth - clientWidth == 0`**（零裁切；`white-space:nowrap` 之下也不可能換行）。
  390px 與 1280px 六組全部零溢出。
- **那 6px 溢出使用者看不到**：`.tblbox` 的 `overflow` 裁切邊界是 **padding box**（根因與註解在
  `index.html:84-89`），右內距 12px，所以末欄右緣 360.48 仍在裁切邊界 366.00 之內、**餘裕 5.52px**
  ——不捲動也完整可見。340~381px 抽樣掃（340／350／355／360／365／370／375／378／380／381）佐證：
  表寬鎖在 min-content 339.48 不再變、溢出量＝`381 − 視窗寬`，而「末欄右緣到裁切邊界」的餘裕在
  370px（溢出 11px）仍為 `+0.52`、要到 365px（溢出 16px）才轉負、內容才真的被裁掉需要橫捲。
  換句話說**溢出量只要不超過那 12px 右內距，畫面就一格都不會少**。
- **與前一批驗收 4px 的差異已查明**：那批以另一組 fixture 量到 4px／門檻 379px，本批以本機
  `BK_NM` 最長股名量到 6px／門檻 381px。**差異全來自第一欄股名長度**，結論方向相同。
  所以**不要把「4px」或「6px」當成常數**——會動的是門檻，不動的是「整頁無捲軸、零裁切、
  溢出落在 12px 右內距內」這三件事。

### 6 位數張數的發生頻率（實查，非估計）

以 `taiwan-flows` 的 `data/daily/`（`origin/main`）**近 29 個交易日＝2026-07-31 ~ 2026-09-09**
逐日掃全市場 `f_net`／`t_net` 絕對值：**8 天（27.6%）出現 ≥100,000 張**，最大為
`00981A` 於 **2026-07-31 的 374,476 張**（八天分別為 07-31、08-03、08-05、08-12、08-14、
08-28、09-01、09-07）。**6 位數不是罕見尾端**，所以上表最後一列是必須寫進驗收條件的常態情境，
不是邊角案例。

> **一個未驗證的前提（引用上述頻率時必須一起講）**：那些張數是拿 `taiwan-flows` 的
> `f_net`／`t_net` 當**代理推估**的。線上 `market_daily` 的 `f`／`t` **至今從未出現過非 `null` 值**
> ——兩個帶 `market_daily` 的資料 commit（`44ef7e7`／`72b3f51`）皆為 `f` 0/2757、`t` 0/2757 非 null。
> 「兩者同單位（張）、同量級」屬**推定、無直接證據**。若日後實際落地的單位不同
> （例如股數未除 1000），欄寬結論要整段重驗。

### 決定：接受現狀、只改寫驗收條件（不動 `index.html`）

實測者提了四個修法，裁定選「接受現狀」，理由逐條記下來，免得下次又有人「順手修一下」：

1. **那 4~6px 對使用者完全不可見**：落在 `.tblbox` 的 12px 右內距裡、不捲也看得到、零裁切、
   零換行。修一個看不見的東西，收益是 0。
2. **選項「`@media (max-width:640px)` 內把 `.mtable` 的 padding 收 1px」會縮小手機點擊區**。
   （該選項的效果本批實測過：`padding:5px 6px` → `5px 5px` 讓 min-content 由 339.48 降到
   331.48，**正好回收 8px**＝4 欄 × 左右各 1px × 2，375px 下溢出由 +6 變 0。）
   為看不見的幾 px 換點擊區變小，不划算。
3. 另外兩個選項——**去掉儲存格的「張」字**（`zh()` 是全站共用格式器，屬**跨 tab 影響**）、
   **改成「37.4萬張」縮寫**（**改變數字呈現口徑**）——成本與風險都更高。
4. **若日後真的要修，選項 1（padding 收 1px）是首選**：最小改動、可回收 8px、效果已實測。

驗收條件的正確版本同時寫進 `CLAUDE.md`「驗證方式」節（那裡才是「怎麼驗」的常青位置）。

### 同批清掉上一輪驗收留的 3 條非阻擋建議

1. **`README.md` 的「第五／第六軸在本節末尾兩則」位置講錯**：該節頂層實有 **9 則** bullet
   （實查 `awk` 掃 `^- `：134／137／144／150／161／191／207／224／232 行），第五軸是第 **7** 則、
   第六軸是第 **8** 則，其後還有第 **9** 則「股名對照」。改成**行首形式**的 grep 錨點
   `'^- \*\*第五軸'`／`'^- \*\*第六軸'`（實測各在 `README.md` 唯一命中）＋明講第 9 則不屬六軸
   ——位置語會隨改檔漂移，錨點不會。
   **順帶踩到本 session 的老陷阱**：初稿把錨點寫成裸名 `**第五軸：`，並宣稱它唯一——
   寫完 grep 一跑就是 **2 行**，因為**那句宣稱自己就是第二次命中**（同 `data-mychg`
   4 處／5 處那次的同型第三次）。改用行首形式後才真的唯一，README 也就地註明了這件事。
2. **`CLAUDE.md` 的「（在 `myChgHtml()` 的點擊 handler 內）」不實**：`location.hash = ` 的賦值在
   `renderMyChg()` **之後**那個 document 級委派 listener 裡（`index.html` 第 1262 行
   `function myChgHtml` 起、第 1359 行 `function renderMyChg`、第 1370 行
   `closest(".clk[data-mychg]")`——賦值在 `myChgHtml()` 的函式體之外）。`myChgHtml()` 只吐出帶
   `data-mychg` 屬性的字串。已改成 grep 錨點 `closest(".clk[data-mychg]")`。
3. **`CLAUDE.md` 寫「三方必須逐項對得上」與本檔「待下批同步」自相矛盾**（那裡已承認
   `index.html` 的 ③④ 與 README 的軸3／軸4 序號互換未解）。改成「**集合**必須一致、
   **序號**尚未對齊且指向本檔待辦」，兩處不再打架。
...
- **驗收（Playwright，本機 `http.server`）**：14 tab 逐一點擊 console error 0／pageerror 0；
  ...
  `TAIEX` 走「不是證券代號」；手機三寬度的正確敘述見本檔「（同日修正之五）手機驗收條件更正」節
  （**本節原寫的「375／390／1280 三寬度 `scrollWidth == innerWidth` 不溢出」已被實測推翻**：
  整頁無水平捲軸成立，但 375px＋6 位數張數時 `.tblbox` 本身會溢出數 px）；
