...
#
# 黑名單為什麼還是必要（`nm` 單獨當閘門不夠）：`TaiwanStockInfo` 自己就混了非證券條目——
# `industry_category` 為「所有證券」的權證 36 檔（實測 `710553` 2026-09-08 在
# `TaiwanStockPrice` 有價，會真的漏進來）、以及 `Index`／`大盤` 這種產業別／指數偽代號 32 筆
# （`TAIEX`、`Semiconductor`…）。黑名單只擋這兩類（**ETN 48 檔已於同日改為納入**，見下段），
# 其餘一律放行——**寧可多放幾檔（多幾十列、幾 KB），不可少放一檔**
# （少放＝使用者的持股在底表消失）。**為什麼需要這道過濾見 build_market_daily() 的 docstring。**
#
# 2026-09-09（同日修正之三）**ETN 改為納入**（使用者裁定）：ETN 是**可以被持有的證券**，
# 把它擋在底表外，持有者在前端會看到「查無此代號（已下市/停牌/代號有誤）」——與事實不符。
# 改法＝`0[2-9]` → `0[3-9]`（放行 `02` 開頭的 ETN）並拿掉 `\d{6}U`。以 FinMind
# `TaiwanStockInfo` 全表（2026-09-09 匿名實查 3,147 檔）對跑新舊規則：新規則**多放行 48 檔、
# 恰好就是全部 ETN**（`industry_category` ＝「ETN」28＋「指數投資證券(ETN)」20），
# **沒有任何一檔由通過變成被擋**（零回歸）。
# 註：拿掉 `\d{6}U` 在該表上是**零影響**——全表沒有任何 `\d{6}U` 形狀的代號，FinMind 實際的
#     ETN 代號是 `020041`／`02001L` 這種**不帶 U** 的寫法；真正放行那 48 檔的是 `0[2-9]`→`0[3-9]`。
#     兩者一起改只是讓「黑名單裡不再有任何一條是為了擋 ETN 而存在」這件事在規則上也成立。
#
# **權證仍然必須留在黑名單**：全市場數以萬計（`TaiwanStockPrice` 單日 45,675 列，權證佔絕大多數），
# 納入會讓 `data/postmkt.json` 再爆到 2,719,486 bytes（實測）。於是「本表不涵蓋這個代號」這個狀態
# **必然存在**，前端不可把它說成「查無此代號」——見 README「前端消費 `market_daily` 的必要條件」。
#
# RE_MARKET_CODE：證券代號的字面型態＝數字開頭的英數（擋掉 Index／大盤 偽代號）。
RE_MARKET_CODE = re.compile(r"^\d[0-9A-Z]{2,7}$")
# RE_MARKET_EXCLUDE：非「可持有證券」的商品代號型態黑名單＝**只剩權證**。
#   0[3-9]\d{3}[0-9A-Z] ＝ 6 碼且首兩碼 03–09：上市權證。**刻意不含 00（ETF：0050／006201／
#                          00637L）、01（REIT 受益證券：01002T）、02（ETN：020041／02001L）**
#   7\d{4}[0-9A-Z]      ＝ 上櫃權證（710553／73107P）；4 碼的 7 開頭個股（7718）長度不同不受影響
# 註：`TaiwanStockInfo` 收的 36 檔權證**全部是 `7` 開頭**（2026-09-09 全表實查），`03`–`09` 開頭的
#     上市權證一檔都不在該表內，所以第一條分支對現況是**防禦性**的（真的漏進 nm 時才用得到）。
...
def fetch_twse_oddlot(base_date: dt.date, report: str) -> tuple[str, list]:
    ...
    for back in range(MAX_BACK_DAYS + 1):
        ...
        try:
            r = throttled_get(f"https://www.twse.com.tw/rwd/zh/afterTrading/{report}",
...
def build_lending(date: str, lend_rows: list, margin_rows: list, short_rows: list,
    ...
    for c in codes:
        ...
        # 瘦身（2026-07-24）：市值/金額衍生欄（*_mv_chg、*_net、plat_total*、short_total*、
        # foreign_shares_mv 等）不再落地——一律可由「基礎量 × px（當日收盤價）」重建，
        # 由消費端現算：前端 index.html augmentLending()、摘要管線 build_summary.py
        # _augment_lending()，公式必須與原後端版一致。由捨入後基礎量重建與原值最多差
        # ±0.5張×px（千元級），遠小於顯示精度（億、1-2位小數）。省 ~0.8MB/2.4MB。
        # sys_mv/otc_mv 例外保留：TWSE 官方市值欄（官方收盤口徑），非本管線推算。
        row = {
            "c": c, "n": nm.get(c, ""), "px": px,
            # 兩平台借券餘額（張；fetch_twse_lending()已將股轉張、元轉千元）
            "sys_bal": round(sys_bal_v), "sys_chg": sys_chg, "sys_mv": round(sb.get("mv", 0)),
            "otc_bal": round(otc_bal_v), "otc_chg": otc_chg, "otc_mv": round(ob.get("mv", 0)),
            ...
            # 三大法人買賣超（張數：股數差÷1000；金額衍生欄由消費端 vol×px 重建）
    ...
    # build_margin() 的融資增加/減少/使用率（錨點 grep 'def build_margin('，**2 處命中：
    # 本註解這一行＋宣告式本身，宣告式才是目標**——在同一個檔裡寫出錨點字串，
    # 「寫出來」這個動作本身就會讓它多一次命中，所以這裡不宣稱「唯一命中」。
    # 排序鍵 chg = bal - prev ＝**融資餘額
...
def build_daytrading(date: str, rows: list, price_rows: list, nm: dict) -> dict:
    ...
    # by_ratio（比重榜）2026-07-24 停產：前端與摘要管線只消費 by_amount，比重榜
    # 從未被渲染；停產同時把分點推估查詢從「兩榜聯集 ~100 檔」減半為金額榜 50 檔

    codes = sorted({x["c"] for x in by_amount})
    if date and codes:
...
def build_market_daily(date: str, price_rows: list, inst_rows: list, inst_date: str, nm: dict) -> dict:
    ...
    **宇宙＝當日 TaiwanStockPrice ∩ TaiwanStockInfo −（權證／指數偽代號黑名單）**
    ...
    - **代號型態只當黑名單**（`RE_MARKET_CODE` 要求數字開頭、`RE_MARKET_EXCLUDE` 擋權證）：
      因為 `nm` 自己就混了非證券條目（權證 36 檔、產業別／指數偽代號 32 筆，2026-09-09 全表
      實查），`nm` 單獨當閘門擋不住。取捨方向固定：**寧可多放幾檔，不可少放一檔**。
    - **ETN 已納入**（2026-09-09 同日修正之三，使用者裁定）：ETN 是可以被持有的證券，
      48 檔全部在宇宙內。詳見 RE_MARKET_EXCLUDE 上方的對跑實證。
    - **前端必要條件（管線端解決不了，必須由消費端配合）**：權證仍留在黑名單（4.5 萬檔，
      納入會讓檔案再爆到 2.7MB），所以**「本表不涵蓋這個代號」這個狀態必然存在**。管線端
    ...
    **與這道守門無關**。保留的理由是本函式作為獨立函式仍可能被其他呼叫端／未來重構以不同的
    ...
    for r in sorted(price_rows, key=lambda x: str(x.get("stock_id") or "")):
        ...
        if c not in nm or not RE_MARKET_CODE.match(c) or RE_MARKET_EXCLUDE.match(c):
            continue  # 權證／指數偽代號：不是可持有的證券，權證更是體積爆炸的來源（ETN 已納入）
        seen.add(c)
        it = inst_by_c.get(c)
        rows_out.append([
            c,
